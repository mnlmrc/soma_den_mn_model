...
class Config:
    ...
    gamma: float = 0.1 # Strength of the PIC (low=0.15, medium=0.3)
    ...
    Tmax: float = 1 * mM # Maximum transmitter concentration
    alpha_exc: float = 0.5 / (ms * mM) # Excitatory synaptic activation rate
    beta_exc: float = 2.5 / ms # Excitatory synaptic deactivation rate
    pulse_width_exc: float = 0.2 * ms # Excitatory synaptic pulse width
    alpha_inh: float = 0.5 / (ms * mM) # Inhibitory synaptic activation rate
    beta_inh: float = 1.2 / ms # Inhibitory synaptic deactivation rate
    pulse_width_inh: float = 0.4 * ms # Inhibitory synaptic pulse width

    # Variables just to initialise the properties - do not correspond to MN values

    # Soma and dendrites sizes (cylindrical volume)
    r_soma_range: List[float] = [0, 0] * um # Soma radius
    l_soma_range: List[float] = [0, 0] * um # Soma length
    r_den_range: List[float] = [0, 0] * um # Dendrite radius
    l_den_range: List[float] = [0, 0] * mm # Dendrite length
 
    I_rheo_range: List[float] = [0, 0] * nA # Rheobase current
    ax_cv_range: List[float] = [0, 0] * meter / second # Axonal conduction velocity

    # Resistances
    R_soma_range: List[float] = [0, 0] * kohm * cm ** 2 # Soma resistance
    R_den_range: List[float] = [0, 0] * kohm * cm ** 2 # Dendrite resistance

    # Soma conductivity properties
    g_kf_range: List[float] = [0, 0] * msiemens / cm ** 2 # Fast potassium conductance per area
    g_ks_range: List[float] = [0, 0] * msiemens / cm ** 2 # Slow potassium conductance per area
    
    alpha_H_range: List[float] = [0, 0] / ms 
    beta_H_range: List[float] = [0, 0] / ms 
    alpha_N_range: List[float] = [0, 0] / ms 
    beta_N_range: List[float] = [0, 0] / ms 
    alpha_Q_range: List[float] = [0, 0] / ms 
    beta_Q_range: List[float] = [0, 0] / ms 
 
    # Dendrite conductivity properties
    g_ca_range: List[float] = [0, 0] * msiemens / cm ** 2 # Calcium conductance per area
    alpha_P: float = 0 / ms
    beta_P_range: List[float] = [0, 0] / ms
    v_thr_ca_range: List[float] = [0, 0] * mV # Calcium threshold for PIC activation

    # Force properties
    Apeak_range: List[float] = [0, 0] * newton # Twitch amplitude
    tpeak_range: List[float] = [0, 0] * ms # Time to peak twitch
